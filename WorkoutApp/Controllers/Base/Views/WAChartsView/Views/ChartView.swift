...
///layoutIfNeeded() - чтобы лаяут был отрисован до того как мы рисуем drawDashLines
        layoutIfNeeded()
        drawDashLines()
...
private extension ChartView {
    ///Количество линий
    func drawDashLines(with lineCounts: Int = 9) {
        (0..<lineCounts).map { CGFloat($0) }.forEach {
            drawDashLine(at: bounds.height / CGFloat(lineCounts) * $0)
        }
    }
    ...
    func drawChart(with data: [WAChartsView.Data], topChartOffset: Int) {
        ...
        ///chartLayer.lineJoin - чтобы не было острых краев на изломе
        chartLayer.lineJoin = .round
        
        layer.addSublayer(chartLayer)
    }
    
    ///Отрисовка точек (Путь и отрисовка)
